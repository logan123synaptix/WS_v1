...
/* --- Console go tay AT qua UART6, forward sang SIM (UART1) ---
 * Thay the cho spam tu dong "AT" moi 2s (theo yeu cau nguoi dung: bo
 * spam, chi go tay). Dung chung 1 ring_buf_t da co san (khai bao ben
 * tren), chi them 1 bien ring_buf_t + 1 isr_byte moi cho UART6, giong
 * het pattern SIM/GPS da chung minh chay dung o phien truoc. */
static ring_buf_t console_ring = {0};
static uint8_t console_isr_byte;

/* Gom byte console (UART6) thanh 1 dong lenh hoan chinh (ket thuc boi
 * '\r' hoac '\n'), roi gui nguyen dong + "\r\n" xuong SIM (UART1). AT
 * command bat buoc phai co \r\n o cuoi (xem Documents/a76xx_at_cmd.md),
 * nen luon tu them vao day, khong bat nguoi dung phai go tay \r\n. */
#define CONSOLE_LINE_BUF_SIZE 128
static char     console_line_buf[CONSOLE_LINE_BUF_SIZE];
static uint16_t console_line_len = 0;
...
static void console_line_send(void);
...
/* Gui dong lenh nguoi dung vua go (console_line_buf) xuong SIM qua
 * UART1, tu dong them "\r\n" o cuoi. Local echo qua UART6 truoc do
 * (trong uart_test_poll(), tung byte 1) da cho nguoi dung thay minh go
 * gi roi, nen o day chi log lai [CONSOLE TX] de doi chieu voi [SIM RX]
 * phia duoi cho de theo doi thu tu. */
static void console_line_send(void)
{
  if (console_line_len == 0) return; /* dong rong (Enter suong), bo qua */

  console_line_buf[console_line_len] = '\0';
  log_info(TAG, "[CONSOLE TX] %s", console_line_buf);

  send_byte_sim(console_line_buf);
  send_byte_sim("\r\n");
...
void uart_test_init(void)
{
  ...
  log_info(TAG, "=== UART TEST -- SIM(UART1) console qua UART6, go AT roi Enter de gui ===");
}
...
void uart_test_poll(void)
{
  ...
  /* Rut can console (UART6) - nguoi dung go tay AT command, ket thuc
   * bang \r hoac \n (Enter). Local echo tung byte ngay lap tuc de nguoi
   * dung thay minh dang go gi tren terminal (khong co echo cung nhu go
   * mu, khong biet da go dung chua truoc khi nhan Enter). */
  while (ring_pop(&console_ring, &byte)) {
    HAL_UART_Transmit(&huart6, &byte, 1, 10); /* local echo */
    if (byte == '\r' || byte == '\n') {
      console_line_send();
    } else if (console_line_len < CONSOLE_LINE_BUF_SIZE - 1) {
      console_line_buf[console_line_len++] = (char)byte;
    } else {
      console_line_send(); /* dong qua dai, gui truoc khi mat du lieu */
    }
  }
}
...
int main(void)
{
  ...
  HAL_UART_Receive_IT(&huart6, &console_isr_byte, 1); /* console go tay AT */
  uint32_t last_tick = 0;
  HAL_RTCEx_DeactivateWakeUpTimer(&hrtc);
  // sx_board_init();
  // app_init();
  uart_test_init();     /* console AT qua UART6, khong con spam tu dong */
  power_on_sim();       /* bat nguon SIM truoc khi go AT tay */
  // power_on_gps();      /* THEM MOI - bat nguon + tha reset GPS, neu khong GPS se
  //                          bi giu o trang thai reset vinh vien (xem comment
  //                          trong power_on_gps()) */
  // test_flash_w25q128(); /* TAM BO - tap trung console AT truoc, xem SIM console */
  // test_imu_bno055();    /* TAM BO - tap trung console AT truoc, xem SIM console */
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  ...
  if (huart->Instance == USART1) {
    ...
    ring_push(&console_ring, console_isr_byte);
    HAL_UART_Receive_IT(&huart6, &console_isr_byte, 1);
  }
}
/* USER CODE END 4 */
